...
//                                                        silokInfo.get(0).getContent() +silokInfo.get(1).getContent()+silokInfo.get(2).getContent(),name)); 실제로 입력할때
        switch(name) {
            case "박세채(朴世采)": {
                lifesummary.setContents("박세채는 반남 박씨 가문에서 출생하여, 조선 시대의 교육 기관인 성균관에서 학문을 시작하였습니다. 그는 이이(李珥)의 학문에 깊은 존경심을 가지고 있었으며, 이는 그가 훗날 학자로서 나아가는 데 중요한 밑거름이 되었습니다. 젊은 시절에는 유명한 학자들의 문하에서 수학하여 지식과 식견을 넓혔고, 이후 학자이자 정치인으로서의 삶을 시작하였습니다.\n" +
                        "\n" +
            ...
            case "이산해(李山海)": {
                ...
                lifesummary.setContents("이산해는 조선 중기의 중요한 관리로서 홍문관 정자, 사헌부집의, 영의정 등을 역임했습니다. 그의 생애는 국가의 재정과 국내외의 외교 등 다양한 무대에서 일어났으며, 그의 행정 역량은 선조의 강력한 지배 아래에서 피어났습니다. \n" +
                        "\n" +
