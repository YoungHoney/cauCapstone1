...
//                                                        silokInfo.get(0).getContent() +silokInfo.get(1).getContent()+silokInfo.get(2).getContent(),name)); 실제로 입력할때
        switch(name) {
            case "박세채(朴世采)": {
                lifesummary.setContents("박세채는 반남 박씨 가문에서 출생하여, 성균관에서 학문을 시작하였습니다. 그는 이이(李珥)의 학문에 깊은 존경심을 가지고 있었으며, 이후 학자이자 정치인으로서의 삶을 시작하였습니다.\n" +
                        "\n" +
                        "1659년에는 중요 관직에 천거되어 익위사세마(翊衛司洗馬)가 되었으며, 당시 정치 상황에서 서인 측의 이론가로 활약하며 남인에 맞서 주장을 펼쳤습니다. 특히 복제사의(服制私議)를 통해 남인의 주장을 체계적으로 반박하면서 자신의 지식과 논리력을 드러냈습니다.\n" +
                        "\n" +
                        "박세채는 소론의 영도자로서 논리적이고 정의로운 입장을 견지하며, 이이와 성혼에 대한 문묘종사 문제의 해결에 기여했을 뿐만 아니라, 대동법과 같은 중요한 정책의 실시를 주장하여 사회 및 경제 개혁에도 큰 영향을 미쳤습니다.\n" +
                        "\n" +
                        "마지막으로, 박세채는 학문적으로나 정치적으로 중요한 발자취를 남긴 인물로서, 후세에 큰 존경을 받고 있는 인물입니다. 그의 삶은 신념과 학문, 그리고 불굴의 정신으로 가득 찬 조선 시대의 선비의 모습을 보여줍니다.test2");
            ...
            case "권응수(權應銖)": {
                ...
                lifesummary.setContents("권응수는 조선시대 뛰어난 무신과 의병장으로, 별시무과에 급제하여 여러 고을의 의병장을 이끌며 고향을 지키는데 주력하였습니다. 임진왜란이 일어났을 때, 권응수는 의병을 모집하여 활발히 활동하였고, 영천성을 화공으로 대승해 수복하는 등 여러 전과를 올렸습니다. \n" +
                        "\n" +
            ...
            case "이산해(李山海)": {
                ...
                lifesummary.setContents("이산해는 조선 중기의 중요한 관리로서 홍문관 정자, 사헌부집의, 영의정 등을 역임했습니다. 그의 행정 역량은 선조의 강력한 지배 아래에서 피어났습니다. \n" +
                        "\n" +
                        "그는 학문에 뛰어나서 어린 나이에 진사가 되었으며, 그 후 급속히 승진하였습니다. 그는 선조의 즉위년에 명나라 사신을 맞이하였고, 다수의 고위 직책을 맡았습니다. \n" +
                        "\n" +
                        "그는 특히 서인과의 정치적인 대립 속에서 핵심 역할을 하였습니다. 그는 영의정에 올라 서인들을 능가하였고, 그는 조선 정부의 핵심 인물이 되었습니다.\n" +
                        "\n" +
                        "그는 또한 문학과 예술에서도 뛰어난 재능을 보였습니다. 그는 선조조 문장팔가 중 한 사람으로 불렸고, 그의 서예실력이 뛰어나서 경복궁 대액의 글씨를 쓴 적도 있습니다.\n" +
                        "\n" +
                        " 이산해는 조선 중기의 주요한 정치적 인물로서 많은 역할을 하였습니다. 그는 국내외 정치, 그리고 문학과 예술에 이르기까지 그의 역량은 상당했습니다. 그는 조선 역사에서 중요한 위치를 차지하고 있습니다.\n");
...
//                silokInfo.get(0).getContent() +silokInfo.get(1).getContent()+silokInfo.get(2).getContent(),name));
        switch(name) {
            case "박세채(朴世采)": {
                mbti.setMbti("ISFJ");
                mbti.setContents("박세채의 기록을 통해 그의 성격 측면을 분석해 이해하려 합니다:\n" +
                        "\n" +
                        "1. 박세채는 뛰어난 학자였으며 학문을 사랑하였습니다. 이는 그가 내향적인 성격(I: Introverted)을 가진 사람일 수 있다는 신호를 줍니다.\n" +
                        "2. 그는 지속적으로 중요한 관직을 역임했고, 조정 운영에 참여하며 정치적 고난을 적절히 극복했습니다. 이러한 행동은 그가 합리적인 판단력을 가지고(J: Judging) 계획 및 조직 능력이 뛰어났다는 것을 보여줍니다.\n" +
                        "3. 그는 파당적 대립을 극단적으로 경계했고, 복잡한 정치 현실에서 자신의 사상을 지키려 했습니다. 이는 그가 확고한 내적 가치관을 공유하며, 그를 기반으로 판단하였음을 나타냅니다.\n" +
                        "4. 박세채는 지속적으로 도덕적인 행동을 강조하였고, 이는 그가 F(Feeling)유형에 속했을 수 있다는 증거를 제공합니다. 그의 접근 방식은 감정과 가치를 중요시하며, 행동을 결정하는데 인간적인 요소를 고려했습니다.\n" +
                        "\n" +
                        "따라서 박세채의 MBTI 유형은 'ISFJ'로 추정됩니다. 이 유형은 굉장히 관찰적이며 세부에 집중하는 경향이 있습니다. 또한, 이들은 전통을 중시하고 사회 규칙과 책임을 매우 중요하게 생각합니다. ");
            ...
            case "김상익(金尙翼)": {
                mbti.setMbti("ESTJ");
                mbti.setContents("김상익의 기록을 통해 몇 가지 성격적 특성을 추측해 볼 수 있습니다:\n" +
                        "\n" +
                        "그는 도승지, 지경연사, 강화유수 등 다양한 직책을 역임했습니다. 이는 그가 새로운 경험과 변경에 개방적이며, 다양한 환경에서 작업하는 능력을 가졌음을 나타냅니다.\n" +
                        "그의 호명이 \"복문\"이라는 사실은 그가 가정 생활에 매우 중점을 둔 것으로 보입니다. 이는 그가 편안하고 안정적인 환경을 선호하며, 전통적인 가치를 중요시했을 것임을 나타내는 특성입니다.\n" +
                        "그는 자신의 의견을 명확하게 표현하고 국상과 관련된 문제에 대해 열정적으로 논의했습니다. 이는 공정하고도 냉철한 판단력을 가진 이성적인 사람을 보여줍니다.\n" +
                        "마지막으로 그는 각별한 존중을 받았습니다. 이는 그가 사람들의 존경을 누렸으며, 그의 리더십이 도덕적으로 적절했음을 나타냅니다.\n" +
                        "\n" +
                        "이러한 특성을 종합해 보면, 김상익은 MBTI에서 E(Extraverted, 외향적), S(Sensing, 센싱), T(Thinking, 사고), J(Judging, 판단) 유형에 해당할 가능성이 있습니다. 예를 들어 ESTJ 유형은 '실천가' 혹은 '관리자'로 불리며, 사회적인 네트워크를 중요시하며, 사물을 현실적이고 명확하게 이해하고 그에 따른 결정을 내리며,질서 정연하고 체계적인 환경을 선호하는 경향이 있습니다. 이 특징들은 김상익이 보여주었던 행동과 태도와 일치하는 경향을 보입니다.");
            ...
            case "권응수(權應銖)": {
                mbti.setMbti("ESTJ");
                mbti.setContents(" 권응수의 기록들을 통해 그가 가진 성격에 대해 몇 가지 추축을 할 수 있습니다:\n" +
                        "\n" +
                        "그는 여러 곳에서 고위 직위를 역임하면서 결단력과 능력을 보여주었습니다. 적과의 전투에서도 끊임없이 성공하고, 그의 개인적인 행동이 전투의 승리에 큰 역할을 한 것으로 나타납니다. 이는 그가 목표 지향적이고 이해력이 뛰어나며 분석적인 사고를 잘 가진 사람일 가능성을 나타냅니다.\n" +
                        "전쟁 상황에서도 그는 팀을 이끌고 전략을 세워 보여준 것으로 볼 때, 그는 리더십 능력을 보유했을 가능성이 있습니다.\n" +
                        "그는 지속적으로 성과를 내고 효과적인 전략을 실행에 옮기는 데 고귀했습니다. 이는 그가 결정적이고 실용적으로 사고하고 행동하며 임무를 완수하는 데 집중하는 성향을 가진 사람일 가능성을 보여줍니다.\n" +
                        "\n" +
                        "이러한 특성을 감안할 때, 권응수는 MBTI에서 E(외향적), S(감각적), T(사고형), J(판단형)에 가깝다고 볼 수 있습니다. 그러므로 그의 추정 MBTI 유형은 ESTJ 일 것입니다. '관리자'로 불리는 이 유형의 사람들은 방향성이 있으며, 체계적이고 사회적으로 책임감을 가지고 있으며, 훌륭한 관리 능력을 가진 것으로 잘 알려져 있습니다. 이는 권응수가 보여준 행동과 잘 일치합니다.");
            ...
            case "이산해(李山海)": {
                mbti.setMbti("ENTJ");
                mbti.setContents("이산해는 관직을 거듭 올라가며 많은 직책을 역임하였으며 또한 선조와 명종의 신뢰를 받았습니다. 그의 정치적 경력에서 느낄 수 있는 그의 결단력과 냉철함, 그리고 유연성은 자신의 관점을 분명하게 표현하고 지키려는 의지를 보여줍니다.\n" +
                        "\n" +
                        "이산해는 비록 일부 충돌이 있었지만, 그는 여전히 자신의 생각과 견해를 표현하는 데 결단력이 있었습니다. 이는 그가 자신의 내부 세계를 절제하고 조절하며 자신의 가치와 신념에 따라 행동했음을 시사합니다. 그는 또한 선조와 명종의 신뢰를 받을 정도로 뛰어난 리더십 능력을 보여주었습니다. 그는 이러한 능력을 바탕으로 복잡하고 어려운 정치적 환경 속에서도 효과적으로 나아가며 목표를 달성하는 데 성공하였습니다.\n" +
                        "\n" +
                        "또한, 그의 폭넓은 경험은 그에게 문제 해결에 필요한 기반 지식과 통찰력을 제공했다고 생각할 수 있습니다. 그는 이러한 지식과 지혜를 이용해 많은 정치적 과제와 위기에 대처하였으므로 그의 의사 결정 프로세스는 실용적이며 결과 지향적이었습니다.\n" +
                        "\n" +
                        "이런 특성을 바탕으로 이산해의 MBTI 유형을 예측해보면 'ENTJ'(외향적, 직관적, 사고적, 판단적)나 'ESTJ'(외향적, 감각적, 사고적, 판단적)로 볼 수 있습니다. ENTJ 유형은 '지도자' 혹은 '사령관'으로 불리며, 큰 그림을 보는 능력과 타인을 이끌어나가는 리더십 능력을 보유하고 있습니다. 한편, ESTJ는 ‘관리자’로 평가받으며 실질적이고 사실에 바탕을 둔 접근법으로 계획을 세우고, 그 계획을 체계적이고 결정적인 방식으로 실행하는 능력을 가지고 있습니다. 이러한 유형들은 이산해의 행동양식과 일치하는 경향이 있습니다");
